...
# --- Configuration ---
OUTPUT_SQL_FILE = "sample_data_low_simulated2025.sql" # New output file name

# Product and Raw Material Definitions with INCREASED INITIAL STOCK for Raw Materials
# but LOW INITIAL STOCK for Finished Goods to trigger production needs.
BAHAN_BAKU_DEFS = [
    {'id': 1, 'nama': 'Kain Katun Polos', 'satuan': 'meter', 'initial_stok': 3000.0, 'harga': 35000.0},
    {'id': 2, 'nama': 'Kancing Plastik Putih', 'satuan': 'lusin', 'initial_stok': 2000.0, 'harga': 18000.0},
    {'id': 3, 'nama': 'Badge Sekolah Bordir', 'satuan': 'pcs', 'initial_stok': 5000.0, 'harga': 5000.0}
]
# CONFIRMED CHANGE: Initial stock for produk_jadi is low to test calculation logic.
PRODUK_JADI_DEFS = [
    {'id': 1, 'kategori': 'Baju Olahraga', 'initial_stok': 10, 'harga': 240000.0},
    {'id': 2, 'kategori': 'Baju Putih Abu Abu', 'initial_stok': 5, 'harga': 270000.0},
    {'id': 3, 'kategori': 'Baju Batik', 'initial_stok': 8, 'harga': 290000.0},
    {'id': 4, 'kategori': 'Baju Pramuka', 'initial_stok': 12, 'harga': 200000.0}
]
RESEP_PRODUK_DEFS = [
...
END_DATE = datetime(2025, 12, 31) # Updated to 2025 to simulate data backward from 2025 to 2023
...
for day_offset in range(DAYS_IN_PERIOD):
    ...
    if day_offset > 0 and day_offset % 7 == 0:
        ...
        purchase_qty = random.randint(200, 500)
    ...
    if day_offset > 0 and day_offset % 10 == 0:
        ...
        production_qty = random.randint(50, 150)
...
print("5. Test the API again with Postman. You should now see positive `quantity_to_make` values.")
